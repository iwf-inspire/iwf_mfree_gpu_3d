...
src/Benchmarks/%.o: ../src/Benchmarks/%.cu
	...
	/usr/local/cuda-8.0/bin/nvcc -O3 -std=c++11 -gencode arch=compute_30,code=sm_30 -gencode arch=compute_60,code=sm_60  -odir "src/Benchmarks" -M -o "$(@:%.o=%.d)" "$<"
	/usr/local/cuda-8.0/bin/nvcc -O3 -std=c++11 --compile --relocatable-device-code=true -gencode arch=compute_30,code=compute_30 -gencode arch=compute_60,code=compute_60 -gencode arch=compute_30,code=sm_30 -gencode arch=compute_60,code=sm_60  -x cu -o  "$@" "$<"
